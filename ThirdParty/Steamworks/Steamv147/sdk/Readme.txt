...
ISteamInventory
* GetItemsWithPrices and GetItemPrice - Added the ability to get the 밷ase price?for a set of items, which you can use to markup in your own UI that items are 뱋n sale?
...
* CreateQueryAllUGCRequest - Added ability to page through query results using a 밹ursor?instead of a page number.  This is more efficient and supports 밺eep paging?beyond page 1000.  The old version of CreateQueryAllUGCRequest() that takes a page parameter is deprecated and cannot query beyond page 1000.  Note that you will need to keep track of the 뱎revious?cursor in order to go to a previous page.
...
* Added a set of functions to ISteamFriends which allow games to integrate with Steam Chat. Games can both join group chats, as well as get friends chats and show them in-line in the game. This API isn뭪 currently used in a game, so there may be some rough edges around the user experience to work out, and some experimentation is required.
